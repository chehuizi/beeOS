"""MonthCloseBox 数据适配器 - M0 hardcoded 7 个工具。

每个函数：
- 纯数据 IO（无决策）
- 返回 Pydantic 兼容的 dict
- 不依赖 LLM / 不依赖外部服务

V1+ 替换为真实 adapter（金蝶/用友 HTTP API / 数据库直连），签名保持不变。
"""
...
def bank_reconcile(period: str) -> dict:
    """银行对账（M0 hardcoded：42 matched + 2 unmatched）。"""
    return {
        "matched": 42,
        "unmatched": [
...
def expense_classify(vouchers: list[dict]) -> list[dict]:
    """费用按科目分类（M0 hardcoded：每条凭证 → "6601 管理费用"）。"""
    return [
        {
...
def evidence_collect(period: str) -> dict:
    """凭证归集（M0 hardcoded：返回 URL 占位 + 固定 87 个文件）。"""
    return {
...
def signoff_request(report_url: str, approver: str) -> dict:
    """发起审批（M0 hardcoded：返回 signoff_id + pending 状态）。"""
    return {
